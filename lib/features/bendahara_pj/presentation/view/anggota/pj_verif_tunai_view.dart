...
import 'package:persis_app/features/bendahara_pj/presentation/controller/pj_verif_tunai_transaction_controller.dart';
import 'package:persis_app/features/bendahara_pj/presentation/view/tunai/pending_transaction_view.dart';
import 'package:persis_app/features/bendahara_pj/presentation/view/pj_invoice.view.dart';
import 'package:persis_app/features/bendahara_pj/presentation/widgets/sweet_alert_dialog.dart';
...
class _PjVerifTunaiViewPageState extends State<PjVerifTunaiViewPage> {
  ...
  Future<void> _handleConfirmTransaction() async {
    ...
    final shouldProceed = await SweetAlertDialog.showConfirmation(
      ...
      message:
          'Apakah anda yakin membayar bulan $monthLabels sebesar $formattedTotal untuk anggota $memberName?',
      ...
      if (invoiceResult != null) {
        ...
        final invoiceData = PjInvoiceData.fromCreationResult(
          ...
          accByName: widget.controller.lookupMemberName(
            invoiceResult.transaction.accBy ??
                invoiceResult.transaction.verifiedBy,
          ),
        );
            ...
            builder: (context, child) {
              return ListenableBuilder(
                ...
                builder: (context, child) {
                  ...
                  bool isAllPaid = true;
                  for (int i = 1; i <= 12; i++) {
                    if (_itemController.getMonthStatus(i, _selectedYear) !=
                        PjMonthStatus.paid) {
                      isAllPaid = false;
                      break;
                    }
                      ...
                      children: [
                        ...
                        GridView.builder(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            bool isDisabled = false;
                            for (int i = 1; i < month; i++) {
                              final s = _itemController.getMonthStatus(
                                i,
                                _selectedYear,
                              );
                              if (s != PjMonthStatus.paid &&
                                  !_selectedMonths.contains(i)) {
                                isDisabled = true;
                                break;
                              }
                            }
