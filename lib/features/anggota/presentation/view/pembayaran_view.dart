  ...
  PembayaranController _buildPembayaranController(
    ...
    if (anggota.riwayatTransaksi.isNotEmpty) {
      final pending = anggota.riwayatTransaksi
          .where((tx) => tx.status == 'pending' || tx.status == 'tunggakan')
          .toList();

      if (pending.isNotEmpty) {
        ctrl.initPeriode(
          pending.first.description ?? '',
          pending.last.description ?? '',
          anggota.totalTagihan,
        );
      }
    }
            ...
            child: Column(
              children: [
                _buildPaymentMethodCard(
                  ...
                  onTap: () => _navigateTo(context, const TransferBankView()),
                ),
                const SizedBox(height: 20),
                _buildPaymentMethodCard(
                  title: 'QRIS',
                  ...
                  onTap: () => _navigateTo(context, const QrisView()),
                ),
              ],
            ),
          )
