...
// ─── Data dummy ────────────────────────────────────────────────────────────────
// Simulasi database pusat: NPA yang terdaftar tapi belum aktivasi
const List<String> _npaDatabase = ['26150403', '26150404', '26150405'];

// Simulasi akun yang sudah aktif (sudah punya password)
...
class _LoginScreenState extends State<LoginScreen>
          ...
          Container(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 8),
                const Text(
                  'Aktivasi akun baru (belum punya password):',
                  style: TextStyle(fontSize: 11, color: greyText),
                ),
                const SizedBox(height: 4),
                _demoRow('NPA', '26150403', '(belum aktif)'),
                _demoRow('NPA', '26150404', '(belum aktif)'),
                _demoRow('NPA', '26150405', '(belum aktif)'),
              ],
            ),
          ),
        ],
  ...
  Widget _buildAktivasiTab() {
    return Column(
      ...
      children: [
        const Text(
          'Masukkan NPA kamu. Sistem akan mengecek apakah kamu terdaftar sebagai anggota Persis.',
          style: TextStyle(fontSize: 13, color: greyText, height: 1.5),
        ),
        const SizedBox(height: 20),
        _buildLabel('Nomor Pokok Anggota (NPA)'),
        const SizedBox(height: 8),
        _buildTextField(
          controller: _npaController,
          hint: 'Contoh: 26150403',
  ...
  void _handleLogin() {
    ...
    // Cek apakah NPA ini belum diaktivasi
    if (_npaDatabase.contains(input)) {
      _showSnackbar(
        'Akun belum diaktivasi. Silakan aktivasi dulu di tab Aktivasi Akun.',
        isError: true,
      );
      return;
    }

    final correctPassword = _activeAccounts[input];
  ...
  void _handleCekNpa() {
    ...
    // NPA sudah aktif → suruh login
    if (_activeAccounts.containsKey(npa)) {
      _showSnackbar(
        'NPA ini sudah aktif. Silakan login langsung.',
        isError: true,
      );
    ...
    // NPA ada di database pusat → lanjut buat password
    if (_npaDatabase.contains(npa)) {
      Navigator.push(
        context,
        MaterialPageRoute(builder: (_) => BuatPasswordScreen(npa: npa)),
      );
      return;
    }

    // NPA tidak ditemukan sama sekali
    _showSnackbar(
      'NPA tidak ditemukan. Pastikan NPA kamu benar.',
      isError: true,
    );
  }
...
class _BuatPasswordScreenState extends State<BuatPasswordScreen> {
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 8),
            Text(
              'Buat password untuk akun NPA ${widget.npa}',
              style: const TextStyle(
                fontSize: 14,
                color: greyText,
                height: 1.5,
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                onPressed: _isLoading ? null : _handleBuatPassword,
                style: ElevatedButton.styleFrom(
                  backgroundColor: primaryGreen,
                  foregroundColor: Colors.white,
                  padding: const EdgeInsets.symmetric(vertical: 16),
  ...
  void _handleBuatPassword() async {
    final password = _passwordController.text.trim();
    ...
    // Simpan ke dummy active accounts
    _activeAccounts[widget.npa] = password;
      ...
      builder: (_) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            ...
            const SizedBox(height: 8),
            Text(
              'NPA ${widget.npa} sudah aktif. Silakan login dengan NPA dan password yang baru kamu buat.',
              textAlign: TextAlign.center,
              style: const TextStyle(
                fontSize: 13,
                color: greyText,
