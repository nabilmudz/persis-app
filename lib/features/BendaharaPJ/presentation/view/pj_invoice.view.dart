				...
				actions: [
					IconButton(
						...
						onPressed: _controller.isSharing
								? null
								: () async {
										final success = await _controller.shareInvoiceAsImage(_boundaryKey);
										if (!success && mounted) {
					...
					return ListView(
						...
						children: [
							RepaintBoundary(
								key: _boundaryKey,
								child: Container(
									color: const Color(0xFFF4F7FA), // Match background color for screenshot
									child: Column(
										children: [
											Container(
											...
											_InvoiceCard(
												...
												child: Column(
													children: [
														for (var index = 0; index < _controller.invoiceData.items.length; index++) ...[
															_InvoiceLineItemTile(item: _controller.invoiceData.items[index]),
															if (index != _controller.invoiceData.items.length - 1)
									...
									SizedBox(
										...
										child: ElevatedButton.icon(
											onPressed: _controller.isSharing
													? null
													: () async {
															final success = await _controller.saveInvoiceToGallery(_boundaryKey);
															if (success && mounted) {
																ScaffoldMessenger.of(context).showSnackBar(
																	const SnackBar(
																		content: Text('Invoice berhasil disimpan ke galeri.'),
									...
									SizedBox(
										...
										child: ElevatedButton.icon(
											onPressed: _controller.isSharing
													? null
													: () async {
															final success = await _controller.shareInvoiceAsText();
															if (!success && mounted) {
																ScaffoldMessenger.of(context).showSnackBar(
																	SnackBar(
																		content: Text(
