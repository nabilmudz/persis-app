...
class TransactionRemoteDataSource {
  ...
  /// Ambil history transaksi yang difilter berdasarkan region_id.
  /// Digunakan oleh PC untuk hanya mengambil transaksi anggota di bawah
  /// region PC tersebut. Endpoint: GET /transaction?region_id={regionId}
  Future<List<TransactionModel>> getTransactionsByRegion(String regionId) async {
    try {
      final url = '/transaction?region_id=${Uri.encodeQueryComponent(regionId)}';
      ...
      debugPrint('⚠ getTransactionsByRegion: status ${response.statusCode}, fallback to getHistory()');
      return getHistory();
    } catch (e, stack) {
  ...
  Future<List<TransactionModel>> fetchAllExportTransactions() async {
    try {
      ...
      final response = await ApiClient.get(url);
      if (response.statusCode != 200 && response.statusCode != 201) {
        debugPrint('⚠️ fetchAllExportTransactions: status ${response.statusCode}');
        return [];
      }
          ...
          'items': [
            {
              ...
              'amount': (tx['total_amount'] as num?)?.toInt() ??
                  (tx['amount'] as num?)?.toInt() ?? 20000,
              'description':
      ...
      debugPrint('[fetchAllExportTransactions] ${txList.length} transaksi dimuat');
      return txList;
    } catch (e) {
